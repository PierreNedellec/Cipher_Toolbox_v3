...
def capitalise(text, filler):
    return text.upper()

def remove_spaces(text, filler):
    return text.replace(' ','')

def remove_linebreaks(text, filler):
    return text. replace('\n','')

def remove_all_but_letters(text, filler):
    result = ''
    for c in text:
        if inalpha(c):
            result += c
...
def block5_spacing(text, filler):
    blocks = []
